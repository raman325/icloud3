...
class PyiCloudSession(requests.Session):
    ...
    def request(self, *args, **kwargs):
        ...
        logger.warning("_____ PyiCloudSession _____ request _____")
    ...
    def _raise_error(self, code, reason):
        logger.warning("_____ PyiCloudSession _____ raise_error _____")
...
class PyiCloudService(object):
    ...
    def __init__(
        ...
        logger.warning("_____ PyiCloudService ______ __init__ _____")
        if password is None:
        ...
        if os.path.exists(cookiejar_path):
            try:
                logger.warning("_____ PyiCloudService ______ __init/get_session_cookies__ _____")

                self.session.cookies = cookielib.LWPCookieJar(
                        filename=cookiejar_path)
                
                self.session.cookies.load()
                
                ################# open full cookie file
                cookie_file = open(cookiejar_path)
                ...
                logger.debug("cookie_data=%s", cookie_data)
                ################# end
    ...
    def get_session_token(self):
        logger.warning("_____ PyiCloudService ______ get_session_token _____")
        self.clientID = self.generateClientID()
        widgetKey = self.setupiCloud.requestAppleWidgetKey(self.clientID)
        
        token = self.idmsaApple.requestAppleSessionToken(self.user['apple_id'],
                                                        self.user['password'],
                                                        widgetKey
                                                        )
 
        logger.debug("session_token=%s", token)
        return token
    
    ################# get the 2sv token 
    def get_2sv_token(self):
        logger.warning("_____ PyiCloudService ______ get_2sv_token _____")
        self.clientID = self.generateClientID()
        widgetKey = self.setupiCloud.requestAppleWidgetKey(self.clientID)
        
        twosv_token = self.idmsaApple.requestApple2SVToken(self.user['apple_id'],
                                                        self.user['password'],
                                                        widgetKey
                                                        )
 
        logger.debug("twosv_token=%s", twosv_token)
        return twosv_token
    ################# end
    ...
    def authenticate(self):
        ...
        logger.warning("_____ PyiCloudService ______ authenticate _____")
        ...
        ################# get the 2sv token 
        twosv_token = self.get_2sv_token()
        ################# end 
        ...
        ################# replace data = { above with following
        #data = {
        #        'accountCountryCode': "GBR",
        #        'extended_login': False,
        #        'TrustToken' : twosv_token,
        #        'dsWebAuthToken': sess_token
        #       }
        ################# end
        
        try:
            logger.warning("_____ PyiCloudService ______ authenticate/session.post _____")
        ...
        logger.warning("_____ PyiCloudService ______ authenticate/saving cookies _____")
    ...
    def generateClientID(self):
        logger.warning("_____ PyiCloudService ______ generateClientID _____")
    ...
    def _get_cookiejar_path(self):
        ...
        logger.warning("_____ PyiCloudService ______ _get_cookiejar_path _____")
        ...
        logger.debug("cookie_jar_path=%s", cookie_jar)
        return cookie_jar

    @property
    def requires_2sa(self):
        """ Returns True if two-step authentication is required."""
        logger.warning("_____ PyiCloudService ______ requires_2sa _____")
        
        b2sa = self.data.get('hsaChallengeRequired', False) \
            and self.data['dsInfo'].get('hsaVersion', 0) >= 1
        
        logger.debug("requires_2sa=%s", b2sa)
        return b2sa
    ...
    def trusted_devices(self):
        ...
        logger.warning("_____ PyiCloudService ______ trusted_devices _____")
    ...
    def send_verification_code(self, device):
        ...
        logger.warning("_____ PyiCloudService ______ send_verification_code _____")
    ...
    def validate_verification_code(self, device, code):
        ...
        logger.warning("_____ PyiCloudService ______ validate_verification_code _____")
        device.update({
    ...
    def devices(self):
        """ Return all devices."""
        logger.warning("_____ PyiCloudService ______ devices _____")
    ...
    @property
    def account(self):
        logger.warning("_____ PyiCloudService ______ account _____")
...
class IdmsaAppleService(HTTPService):
    def __init__(self, session):
        ...
        self.apple2SVToken = None
        ################# end
        
    def requestAppleSessionToken(self, user, password, appleWidgetKey):
        logger.warning("_____ IdmsaAppleService ______ requestAppleSessionToken _____")
        ...
        try:
            ...
            err_str = "request AppleSessionToken: " + \
    ...
    ################# start
    def requestApple2SVToken(self, user, password, appleWidgetKey):
        logger.warning("_____ IdmsaAppleService ______ request2SVToken _____")
        #self.session.headers.update(self.getRequestHeader(appleWidgetKey))
        header = self.getRequestHeader(appleWidgetKey)
        logger.debug("appleWidgetKey=%s", appleWidgetKey)
        logger.debug("header=%s", header)
        self.response = self.session.get(self.url2SV)

        logger.debug("2SV self.response=%s", self.response)
        try:
            headers = self.response.headers
            self.apple2SVToken = headers["X-Apple-TwoSV-Trust-Token"]
        except Exception as e:
            err_str = "request X-Apple-TwoSV-Trust-Token: " + \
                      "Apple 2SV Token query failed"

            raise Exception(err_str,
                            self.urlAuth, repr(e))
        return self.apple2SVToken
    ################# end
