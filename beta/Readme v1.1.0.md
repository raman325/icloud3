...
| Track device from more than one location | No, can only track from the 'home' zone | Yes, can track from the 'home' zone and other zones (office, second home, etc.) using *base_zone: zonename* parameter. |
| | | |
...
- iCloud3 will not poll the device on a regular basis since it can't access the iCloud Find-My-Friends/Find-my-Phone Location Service. The decreasing interval as you approach home will be not be done. Automations and scripts based on a short distance from home will not trigger. Automations and scripts triggered on a zone change should continue to work.
...
device_tracker:
  - platform: icloud3
    ...
    password: gary_itunes_Password
    group: family                      <<< Name of this group of tracked devices.
    tracked_devices:                   <<< Devices to be tracked
      - gary_iphone > gary_real@email.com, gary.png, 2
      - lillian_iphone > lillian_real@email.com, lillian_real.com, lillian.png, 3
```
...
| ------------------ | ------------------------------------------------------------ |
| email_address      | *Required for the tracking_method: fmf*. FmF uses the email address of the "friends" iCloud account to link it with the FmF account. The email address is linked to the device's entity here.<br><br>You can use the complete email address ```(gary456@gmail.com)``` or just the name part ```(gary456```).<br><br>Not required if you are using the icloud or iosapp tracking methods. (You can omit the comma placeholder) |
| badge_picture_name | The file name containing a picture of the person normally associated with the device. See the sensor_badge section of the iCloud3 documentation for more information about the sensor_badge.<br><br>The file is normally in the ```config/www/``` directory (referred to as '/local/' in HA). You can use the full name ```(/local/gary.png)``` or an abbreviated name ```(gary.png)```. |
...
```yaml
  Examples:
    ...
    - gary_iphone > gary456, gary.png, 1        <<<< IOS App Id v2 ' _1'
    - gary_iphone > gary456, gary.png,, garyc   <<<< '/local/' will be added
    - gary_iphone > ,gary.png, 1, garyc
	- gary_iphone > /local/gary.png
	- gary_iphone > gary.png                    <<<< IOS App v1, IOS App tracking
	- gary_iphone                               <<<< FmPhn tracking method
...
*Format:* 
 ...
 `base_zone: base_zone_name, sensor_prefix` (`base_zone: office`)`

| Field          | Description/Notes                                            |
| -------------- | ------------------------------------------------------------ |
| base_zone_name | Name of the zone, other than the 'home' zone, to base all distance and time calculations on.<br><br>The sensor for the distance to 'home' is `sensor.devicename_home_distance`. When the *base_zone*  is used, the home distance attribute becomes the distance to the *base_zone*. For example, `sensor.office_gary_iphone_home_distance` is the distance to the 'office' zone, not the 'home' zone. |
...
- `lillian_iphone > lillian678, lillian.png`
  No special name is specified so the devicename (*lillian_iphone*) is used as the prefix, e.g.,  `sensor.lillian_iphone_home_distance`
- `gary_iphone > gary456, gary.png,, garyc* `
  A special prefix name (*garyc*) is specified, so that will be used, e.g., `sensor.garyc_home_distance`

If the *base_zone* parameter is specified, it will be added to the sensor name before the devicename. For example:

- *base_zone: whse*  
  No special base zone name is specified so the zone (*whse*) is used as the prefix, e.g.,  `*sensor.whse_lillian_iphone_home_distance`
- *base_zone: whse, vbw*  
  A special base zone name (*vbw*) is specified so that will be used, e.g., `sensor.vbw_garyc_home_distance`
- *base_zone: whse, noprefix*  
  A special base zone name (*"noprefix"*) is specified so no prefix will be used, e.g., `sensor.garyc_home_distance`
...
- `lillian_iphone > lillian678@gmail.com, lillian.png* `
  The *sensor.lillian_iphone_badge* is created with the picture file */local/lillian.png*
- `gary_iphone > gary456, /local/gary.png,, garyc* `
...
- **create_sensors**
  Lets you select only the sensors to be created, e.g., `sensors: zon,zon1,ttim,hdis,nupdt,lupdt,cnt,info` will only create the _zone, _zone_name1, _home_distance, _next_update, _last_update, _poll_cnt, _and _info sensors
- **exclude_sensors**
  All sensors except the ones you specify are created, e.g., `exclude_sensors: znam2,znam3,_lzon2,lzon3,_zon_timestamp,bats` will create all sensors except _zone_name2, _zone_name3, _last_zone2, _last_zone3, _zonts, _battery_status
...
| _home_distance   | hdis  |      | _zone_name2      | zon2  |      | _gps_accuracy          | gpsacc |
...
This service will play the Lost iPhone sound on a specific device. This is not available for the Find-my-Friends tracking methoc.
