...
CONF_TRACK_BY_ALIAS         = 'track_by_alias'
CONF_NAME                   = 'name'
...
SENSOR_ATTR_FORMAT      = {'distance': 'dist', 'calc_distance': 'dist',
                           ...
                           'speed': 'kph-mph', 'speed_high': 'kph-mph', 
...
SENSOR_ATTR_ICON        = {'zone': 'mdi:cellphone-iphone',
                           ...
                           'eneity_log': 'mdi:format-list-checkbox'}
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Optional(CONF_TRACK_BY_ALIAS): vol.All(cv.ensure_list, [cv.string]),
...
def setup_scanner(hass, config: dict, see, discovery_info=None):
    ...
    track_by_alias         = config.get(CONF_TRACK_BY_ALIAS)
...
class Icloud(DeviceScanner):
    ...
    def __init__(self, hass, see, username, password, account,
        ...
        self.see                = see
        self.track_by_alias     = track_by_alias
        self.track_by_alias_flag = {}
        ...
        self.inzone_interval = self._time_str_to_secs(
                                            inzone_interval_str)
        ...
        self.time_last_poll            = {}
        self.time_since_zone_exit      = {}
        self.dist_since_zone_exit      = {}
        self.speed                     = {}
        self.speed_high                = {}
        self.speed_average             = {}
...
#--------------------------------------------------------------------
    def restart_icloud(self):
        ...
        try:
            ...
            self._setup_device_alias(self.track_by_alias)
            self.tracked_devices          = {}
            self.device_type              = {}
            self.overrideinterval_seconds = {}
            ...
            for device in self.api.devices:
                ...
                #If device has an alias, verify it exists, turn off if not 
                self._check_device_alias(devicename)
            ...
            self.event_log_base_attrs    = self._setup_event_log()
            ...
            for devicename in self.tracked_devices:
                ...
                self.state_this_poll[devicename]       = \
                        self._get_current_state(devicename, USE_ALIAS)

                #self.track_by_alias_flag[devicename]  = False        
                ...
                self.time_since_zone_exit[devicename] = 0
                self.dist_since_zone_exit[devicename] = 0
                ...
                attrs[ATTR_ALIAS]      = self.device_alias.get(devicename)
    ...
    def _polling_loop_5_sec_device(self, now):
        ...
        try:
            for devicename in self.tracked_devices:
                ...
                current_state = self._get_current_state(devicename, USE_ALIAS)
                current_state = current_state.lower()
                dev_attrs = self._get_device_attributes(devicename, USE_ALIAS)
    ...
    def _update_device_icloud(self, update_reason = 'CheckiCloud',
        ...
        try:
            for devicename in self.tracked_devices:
                ...
                latitude            = round(float(dev_data[1]), 5)
...
#                   No icloud data, reauthenticate (status=None)
        ...
        try:
            ...
            if location:
                ...
                latitude      = round(location[ATTR_LATITUDE], 5)
...
#########################################################
    ...
    #       the distance will be 0.
        ...
        speed_data = self._get_speed(devicename, dist_last_poll_moved)
        speed         = speed_data[0]
        speed_high    = speed_data[1]
        speed_average = speed_data[2]
        ...
        try:
            ...
            self.last_distance_str[devicename] = '{} {}'.format(
                        dist_from_home, self.unit_of_measurement)
...
#------------------------------------------------------------------------
    def _get_speed(self, devicename, dist_last_poll_moved):
        ...
        try: 
            ...
            if self._was_inzone(devicename):
                ...
                self.speed_average[devicename]        = 0
                self.dist_since_zone_exit[devicename] = 0
                self.time_since_zone_exit[devicename] = self.this_update_seconds
            
            age  = int(self._secs_since(
                        self.time_last_poll.get(devicename))) / 3600
            aget = int(self._secs_since(
                        self.time_since_zone_exit.get(devicename))) / 3600
                       
            log_msg = ("SpeedCalc: ThisSecs({})-TimeLastPol({})=Age({}), "
                "ThisSecs-TimeZoneExit({})=AgeTotal({}), DistLastPoll={}, "
                "DistZoneExit={}").format(
                self.this_update_seconds, 
                self.time_last_poll.get(devicename), age,
                self.time_since_zone_exit.get(devicename), aget,
                dist_last_poll_moved,
                self.dist_since_zone_exit.get(devicename))
            self._LOGGER_debug_msg(log_msg)
            
            self.dist_since_zone_exit[devicename] += dist_last_poll_moved
            self.time_last_poll[devicename] = self.this_update_seconds
            
            if age > 0:
                speed = round(dist_last_poll_moved/age, 0)
                if (speed > self.speed_high.get(devicename)):
                    speed_high = speed
            if aget > 0:
                speed_average = round(
                        self.dist_since_zone_exit.get(devicename)/aget, 0)
                              
            speed         = self._km_to_mi(speed)
            speed_high    = self._km_to_mi(speed_high)
            speed_average = self._km_to_mi(speed_average)

            self.speed[devicename]         = speed
            self.speed_high[devicename]    = speed_high
            self.speed_average[devicename] = speed_average
        
            log_msg = ("Speed={} (Dist/Time={}/{}), Avg={} "
                "(Dist/Time={}/{})").format(speed, dist_last_poll_moved,
                age, speed_average, 
                self.dist_since_zone_exit.get(devicename), aget)  
            self._LOGGER_debug_msg(log_msg)
            self._save_event(devicename, log_msg)
                
        except Exception as err:
            attrs_msg = self._internal_error_msg(fct_name, err, 'SetSpeed')
            speed         = -1
            speed_high    = -1
            speed_average = -1
            #_LOGGER.exception(err)
...
#
#########################################################
    def _get_device_entity_id(self, devicename, use_alias):
        if (use_alias and self.track_by_alias_flag.get(devicename)):
            return self.device_entity_alias.get(devicename)
        else:
            return self.device_entity_id.get(devicename)
            
#--------------------------------------------------------------------
    def _get_current_state(self, devicename, use_alias = False):
        """
        Get current state of the device_tracker entity
        (home, away, other state)
        """

        entity_id = self._get_device_entity_id(devicename, use_alias)
        
        try:
            device_state = self.hass.states.get(entity_id).state
            if (use_alias and self.track_by_alias_flag.get(devicename)):
                _LOGGER.info("%s, %s",devicename,entity_id)
                _LOGGER.info("%s, device_state-%s",devicename,device_state)
            if device_state:
...
#--------------------------------------------------------------------
    def _setup_base_kwargs(self, devicename, latitude, longitude,
        ...
        return kwargs
#--------------------------------------------------------------------
    def _get_device_attributes(self, devicename, use_alias = False):
...
#--------------------------------------------------------------------
    def _setup_sensor_name_devicename(self, devicename):
        ...
        return
#--------------------------------------------------------------------
    def _setup_device_alias(self, devicename_alias):
        '''
        The track_by_alias will associate the alias number created by the 
        ios app to a devicename.
        
            track_by_alias:
              - gary_iphone @ alias
              - lillian_iphone @ alias
              
              ['name', 'gary_iphone @ alias','lillian_iphone @ alias']

        '''

        if devicename_alias:
            for kv in devicename_alias:
                kvw = kv.replace(" ", "", 99)
                if '@' in kvw:
                    dev_name_alias = kvw.replace(' ','',99).lower().split('@')
                    devicename = dev_name_alias[0]
                    alias      = dev_name_alias[1]
                    self.device_alias[devicename] = alias
                    _LOGGER.warning("%s, devalias-%s",devicename,self.device_alias)
                    

#--------------------------------------------------------------------
    def _check_device_alias(self, devicename):
        '''
        The alias was setup for the device from the config file earlier.
        #Not that the devicename is set up, check to make sure the
        #alias is valid. The alias is only used to get data from the ios app,
        #not when the device is polled via iCloud FMF location svcs.
        '''
        self.track_by_alias_flag[devicename] = False
        device_state = None
        _LOGGER.info("%s, devalias-%s",devicename,self.device_alias)
        if self.device_alias and devicename in self.device_alias:
            try:
                alias        = self.device_alias.get(devicename)
                entity_id    = '{}.{}'.format(DOMAIN, alias)
                _LOGGER.info("%s, eid-%s",devicename,entity_id)
                device_state = self.hass.states.get(entity_id)  #.state
                self.track_by_alias_flag[devicename] = True
                self.device_entity_alias[devicename] = entity_id
                _LOGGER.info("%s, device_state-%s",devicename,device_state)           
                log_msg = ('Device {}, Alias-{}').format(devicename, alias)
                self._LOGGER_info_msg(log_msg)
            
            except AttributeError as err:
                _LOGGER.exception(err)
                log_msg = ('AEDevice {}, Invalid alias-{}').format(devicename, alias)
                self._LOGGER_error_msg(log_msg)

            except Exception as err:
                _LOGGER.exception(err)
                log_msg = ('EDevice {}, Invalid alias-{}').format(devicename, alias)
                self._LOGGER_error_msg(log_msg)
...
#########################################################
    def _setup_event_log(self):
        ...
        pict_attrs       = {}
        devicename_attrs = {}
        sensor_entity = "sensor.icloud3_event_log"
            
        try:
            attrs = self.hass.states.get(sensor_entity).attributes
        ...
        attrs = {}
        attrs["names"]       = name_attrs
        attrs["pictures"]    = pict_attrs
        attrs["devicenames"] = devicename_attrs
        attrs["logs"]        = ""
        #_LOGGER.error("New attrs>>%s",attrs)

        self.hass.states.set(sensor_entity, "Initialized", attrs)
...
#------------------------------------------------------
    def _update_sensor_event_log_data(self, devicename = None):
        ...
        try:
            log_attrs  = self.event_log_base_attrs
            attr_recd = ''
            attr_event = {}
            ...
            log_attrs["logs"] = '{}'.format(attr_recd[1:])
...
#------------------------------------------------------
    def _prepare_event_logs(self, devicename):
        ...
        for event_recd in reversed(self.event_log_table):
            er_devicename = event_recd[0]
            if (er_devicename == devicename or er_devicename == '*'):
                log_cnt += 1

                attr_event["time"]  = event_recd[1]  #current time
                attr_event["state"] = event_recd[2]  #state
                attr_event["zone"]  = event_recd[3]  #zone
                attr_event["dist"]  = event_recd[4]  #distance
                attr_event["ttime"] = event_recd[5]  #waze travel time
                attr_event["intvl"] = event_recd[6]  #interval
                attr_event["text"]  = event_recd[7]  #event text

                if attr_event.get("state") == "":
                    attr_event["state"] = "Initialize"
                attr_recd = '{},{}'.format(attr_recd, attr_event)

        #Final record so iCloud3-event-log-card will work
        attr_event["time"]  = '00:00:00'
        attr_event["state"] = ''
        attr_event["zone"]  = ''
        attr_event["dist"]  = ''
        attr_event["ttime"] = ''
        attr_event["intvl"] = ''
        attr_event["text"]  = 'Last Record'
        attr_recd = '{},{}'.format(attr_recd, attr_event)
